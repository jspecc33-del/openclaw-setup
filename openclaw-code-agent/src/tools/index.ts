...
function resolveWorktreeStrategy(input?: string): WorktreeStrategy {
  const valid: WorktreeStrategy[] = ["delegate", "ask", "off", "manual", "auto-merge", "auto-pr"];
  return valid.includes(input as WorktreeStrategy) ? (input as WorktreeStrategy) : "delegate";
}

function resolvePlanApproval(input?: string): PlanApproval {
  const valid: PlanApproval[] = ["delegate", "ask", "off"];
  return valid.includes(input as PlanApproval) ? (input as PlanApproval) : "delegate";
}
...
const agentLaunch = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<Session> => {
...
const agentRespond = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ sent: boolean }> => {
...
const agentRequestPlanApproval = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ escalated: boolean; plan?: string }> => {
...
const agentSendPlanOffer = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ sent: boolean; plan?: string }> => {
    const session = ctx.store.get(args.sessionId);
    if (!session) throw new Error(`Session not found: ${args.sessionId}`);
    if (!session.pendingPlan) throw new Error(`No pending plan for session ${args.sessionId}`);
...
const agentOutput = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ output: string; length: number }> => {
...
const agentKill = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ killed: boolean; sessionId: string }> => {
...
const agentStats = {
  ...
  handler: async (_args: any, ctx: ToolContext): Promise<AgentStats> => {
    const sessions: Session[] = ctx.store.list();
    let totalCostUsd = 0;
...
const agentMerge = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ success: boolean; message: string; sha?: string }> => {
    const session = ctx.store.get(args.sessionId);
    if (!session) throw new Error(`Session not found: ${args.sessionId}`);
...
const agentPr = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ success: boolean; prUrl?: string; message: string }> => {
    const session = ctx.store.get(args.sessionId);
    if (!session) throw new Error(`Session not found: ${args.sessionId}`);
...
const agentWorktreeStatus = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<any> => {
    if (args.sessionId) {
      const session = ctx.store.get(args.sessionId);
      if (!session) throw new Error(`Session not found: ${args.sessionId}`);
      return {
    ...
    return ctx.store.list().map((s: Session) => ({
      sessionId: s.id,
      name: s.name,
...
const agentWorktreeCleanup = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ cleaned: string[]; dismissed?: string[] }> => {
    const cleaned: string[] = [];
    const dismissed: string[] = [];

    if (args.mode === "preview_safe") {
      const sessions: Session[] = args.sessionId
        ? [ctx.store.get(args.sessionId)].filter((s): s is Session => s !== undefined)
        : ctx.store.list();
      for (const s of sessions) {
...
const goalLaunch = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<GoalTask> => {
    const goalType: GoalType = args.type;
...
const goalStop = {
  ...
  handler: async (args: any, ctx: ToolContext): Promise<{ stopped: boolean; goalId: string }> => {
    const engine = ctx.goalEngines.get(args.goalId);
    if (!engine) throw new Error(`Goal not found: ${args.goalId}`);
    await engine.stop();
...
export const toolsByName: Record<string, (typeof allTools)[number]> = Object.fromEntries(
