...
import { PluginConfig, ToolContext } from "./types";
...
// Goals subsystem
import { GoalEngine } from "./goals/base";
...
let monitor: Monitor;
let worktreeIsolation: WorktreeIsolation;
let worktreeFollowThrough: WorktreeFollowThrough;
let worktreeLifecycle: WorktreeLifecycle;
let planApprovalFlow: PlanApprovalFlow;
let planParser: PlanParser;
let wakeRouter: WakeRouter;
let toolContext: ToolContext;
let goalEngines: Map<string, GoalEngine>;
let sessionGoals: Map<string, string>;

// ---------------------------------------------------------------------------
// Harness adapter factory
// ---------------------------------------------------------------------------

function createHarnessAdapter(session: import("./types").Session, config: PluginConfig): HarnessAdapter {
...
export default definePlugin({
  ...
  async onLoad(ctx: any) {
    ...
    const harnessLike = {
      send: (sessionId: string, message: string) => sessionLifecycle.respond(sessionId, message),
    };
    ...
    for (const tool of allTools) {
      ctx?.tools?.register?.(tool.name, (args: any) => tool.handler(args, toolContext));
    }
    ...
    log("[openclaw-code-agent] Plugin loaded. Harness: %s, Mode: %s, Plan: %s, Worktree: %s",
      pluginConfig.defaultHarness,
      pluginConfig.permissionMode,
      pluginConfig.planApproval,
      pluginConfig.defaultWorktreeStrategy,
    );
  },

  async onUnload(ctx: any) {
    const log = ctx?.logger?.info ?? console.log;
