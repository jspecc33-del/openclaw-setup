 ...
 */
export function findSessionByName(store: any, name: string): Session | undefined {
  const sessions: Session[] = store.list();
 ...
 */
function findGoalEngineByName(ctx: ToolContext, name: string): any | undefined {
...
interface CommandHandler {
  (args: string[], ctx: ToolContext): Promise<string>;
}

function makeCommands(ctx: ToolContext): Record<string, CommandHandler> {
  return {
    /**
     * /agent <name> <...instructions>
     * Launch a new background coding session.
     */
    agent: async (args: string[]): Promise<string> => {
      if (args.length < 2) {
     ...
     */
    agent_sessions: async (_args: string[]): Promise<string> => {
      const tool = toolsByName["agent_sessions"];
      const sessions: Session[] = await tool.handler({}, ctx);
      if (sessions.length === 0) {
        return "No sessions found.";
      }
      const lines = sessions.map(
        (s) => `- ${s.name} | ${s.state} | ${s.harness} | ${s.workdir}`
      );
     ...
     */
    agent_output: async (args: string[]): Promise<string> => {
      if (args.length < 1) {
     ...
     */
    agent_respond: async (args: string[]): Promise<string> => {
      if (args.length < 2) {
     ...
     */
    agent_kill: async (args: string[]): Promise<string> => {
      if (args.length < 1) {
     ...
     */
    agent_stats: async (_args: string[]): Promise<string> => {
     ...
     */
    goal: async (args: string[]): Promise<string> => {
      if (args.length < 2) {
      ...
      const task = await tool.handler(
        { name, type: "ralph", target, completionSignal: "DONE" },
        ctx
      );
     ...
     */
    goal_status: async (args: string[]): Promise<string> => {
      ...
      const lines = results.map(
        (g: any) => `- ${g.name} | ${g.type} | ${g.state} | ${g.currentIteration}/${g.maxIterations}`
      );
     ...
     */
    goal_stop: async (args: string[]): Promise<string> => {
      if (args.length < 1) {
 ...
 */
function parseCommandLine(line: string): { command: string; args: string[] } | null {
 ...
 */
export function registerCommands(ctx: ToolContext): (line: string) => Promise<string> {
