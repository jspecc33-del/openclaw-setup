...
export class WorktreeLifecycle {
    ...
    transition(sessionId, to) {
        ...
        if (TRANSITIONS[from].length === 0) {
            throw new Error(`Cannot transition from terminal state "${from}" for session ${sessionId}`);
        }
        // Validate the transition
        const allowed = TRANSITIONS[from];
        if (!allowed.includes(to)) {
            throw new Error(`Invalid transition: "${from}" → "${to}" for session ${sessionId}. ` +
     ...
     */
    initialize(sessionId, initial = "active") {
